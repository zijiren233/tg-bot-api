...
func NewClient(client *http.Client) HTTPClient {
	return &botClient{client: client, limiter: rate.NewLimiter(29, 1)}
}

func (cli *botClient) Do(req *http.Request) (*http.Response, error) {
...
func NewBotAPI(token string) (*BotAPI, error) {
	return NewBotAPIWithClient(token, APIEndpoint, NewClient(&http.Client{}))
}
...
func (bot *BotAPI) GetFileDataWithCtx(ctx context.Context, fileID string) ([]byte, error) {
	File, err := bot.GetFile(FileConfig{FileID: fileID})
	if err != nil {
		return nil, err
	}
	return bot.DownloadFileWithCtx(ctx, File)
}

func (bot *BotAPI) GetFileData(fileID string) ([]byte, error) {
	File, err := bot.GetFile(FileConfig{FileID: fileID})
	if err != nil {
		return nil, err
	}
...
func (bot *BotAPI) DownloadFileWithCtx(ctx context.Context, file File) ([]byte, error) {
...
func (bot *BotAPI) DownloadFile(file File) ([]byte, error) {
	return bot.DownloadFileWithCtx(context.Background(), file)
}
